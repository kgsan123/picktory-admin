...
## 2026-06-27 출연자 명단 자동 추천 (cast_suggest)
**Decision:** data_collector/cast_suggest.py 신규 — 나무위키(출연진/등장인물) + 뉴스 텍스트를 모아 Groq LLM으로 현재 시즌 출연자 실명만 추출. 어드민 [설정]에 "🔎 출연자 자동 추천" 버튼: 결과를 출연자 명단 칸에 채우고, 운영자가 확인·수정 후 [설정 저장]. 예측 생성은 저장된 shows.cast_names만 사용.
**Reason:** 운영자가 출연자를 일일이 찾는 수고 제거. 단 자동 스크래핑/LLM은 100% 보장 불가 → 자동은 "후보 제안"까지만, 100% 정확도는 운영자 저장(확인) 단계에서 보장. 미저장 자동 결과는 예측에 쓰이지 않아 환각 cast 유입 차단.
**Impact:** 신규 의존성 없음(기존 namu fetch + google RSS + Groq 재사용). LLM은 0.0 temp, 자료 내 이름만 추출 지시. 연애실험실 실측: 깨끗한 실명 6명 추출 확인.

## 2026-06-25 잔여 이슈 수정: 외국문자 제거 + 운영자 출연자 명단
**Decision:** (1) `_clean_prediction`을 한자만 제거→한글·영문·숫자·기본문장부호 외 전부 제거(키릴/가나 등)로 확장. (2) shows에 `cast_names` 컬럼(운영자 입력, 마이그레이션 009) 추가. 생성 시 이 명단을 `cast_seed`로 fetch_episode_context에 주입 → [이번 회차 등장인물]·grounding에 사용. 명단 ≥2면 컨텍스트 충분으로 간주(thin-cast 프로그램도 생성 가능). 어드민 설정에 "출연자 명단" 입력칸.
**Reason:** (1) 모델이 가끔 "сегодня"(러시아어) 등 외국문자 섞음. (2) 넷플릭스 연애·서바이벌은 뉴스에 실명이 없어 cast 자동추출 실패→카운트형 예측만 나오고 품질 불안정. 운영자가 명단을 한 번 넣으면 시즌 내내 실명 선택지 확보.
**Impact:** 마이그레이션 009 수동 적용 필요. 009 전에도 생성은 동작(notes만 사용, cast 없음). 자동 추출 cast와 운영자 명단은 병합(운영자 우선).
